...
<div class="content">
<div>
  <h2 style="float: right;"><?php echo $products_price; ?></h2>
...
?>
  ...
  <div style="float: right; width: <?php echo SMALL_IMAGE_WIDTH+20; ?>px; text-align: center;">
    <?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $review['products_id']) . '">' . tep_image(DIR_WS_IMAGES_PRODUCT . $review['products_image'], addslashes($review['products_name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"') . '</a>'; ?>
...
?>
  ...
  <div class="contentText">
    <?php echo tep_break_string(nl2br(tep_output_string_protected($review['reviews_text'])), 60, '-<br />') . '<br /><br /><i>' . sprintf(TEXT_REVIEW_RATING, tep_get_stars($review['reviews_rating']), sprintf(TEXT_OF_5_STARS, $review['reviews_rating'])) . '</i>'; ?>
  </div>

  <br />

  <div class="buttonSet">
    <span class="buttonAction"><?php echo tep_draw_button_booth(IMAGE_BUTTON_WRITE_REVIEW, 'edit', tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, tep_get_all_get_params(array('reviews_id'))), 'primary'); ?></span>

    <?php echo tep_draw_button_booth(IMAGE_BUTTON_BACK, 'chevron-left', tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id'))),'default'); ?>
  </div>
</div>
