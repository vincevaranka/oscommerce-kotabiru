<?php
  class bm_side_manufacturers {
    var $code = 'bm_side_manufacturers';
    var $group = 'boxes';
    var $title;
    var $description;
    var $sort_order;
    ...
    function bm_side_manufacturers() {
      ...
      if ( defined('MODULE_BOXES_MANUFACTURERS_STATUS') ) {
		...
		$this->placement = MODULE_BOXES_MANUFACTURERS_CONTENT_PLACEMENT;
      }
    }
    ...
    function execute() {
     global $HTTP_GET_VARS, $oscTemplate,$PHP_SELF;
		if (isset($HTTP_GET_VARS['products_id'])) 
		{
			...
			$cat_search_query = tep_db_query("select distinct c.categories_id as id, cd.categories_name as name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where p.products_status = '1' and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id and p2c.categories_id = cd.categories_id and p.manufacturers_id = '" . (int)$manufacturer['manufacturers_id'] . "' order by cd.categories_name");
						...
						if (tep_db_num_rows($cat_search_query) > 1) {
							 while ($cat_search = tep_db_fetch_array($cat_search_query)) {
									...
									$category_list .= '<li><a href="'.tep_href_link(FILENAME_DEFAULT, 'manufacturers_id='.$product_info['manufacturers_id'].'?filter_id='.$cat_search['id']).'">' . $cat_name .' ('. tep_count_products_in_category_manufacturers($cat_search['id'],$manufacturer['manufacturers_id']).') </a></li>';	
								}
			...
			$oscTemplate->addBlock($data, $this->group);
					...
					if (tep_db_num_rows($manu_search_query) > 0) {
						...
						while ($manu_search = tep_db_fetch_array($manu_search_query)) {
							...
							$category_list .= '<li><a href="'.$cat_name.$manu_path.'-c-'.$HTTP_GET_VARS['cPath'].'?filter_id='.$manu_search['id'].'">' . $manu_name . '</a></li>';
						}	
						$category_list .= '</ul>';
					
						  $content2 = $category_list;
						
						$data = '<div class="box"><div class="box-heading">' . MODULE_BOXES_MANUFACTURERS_BOX_TITLE . '</div><div class="box-content box-category">'.$content2.'</div></div>';
			...
			$data = '<div class="box"><div class="box-heading">'.MODULE_BOXES_MANUFACTURERS_CATEGORY_BOX_TITLE.'</div><div class="box-content box-category"><strong>'.tep_manufacturers_name($mID).'</strong> : <ul>'.$categories_string.'</ul></div></div>';
			$oscTemplate->addBlock($data, $this->group);
		 }
		...
		//		$manufacturers_string .= '<li><a href="' . tep_href_link(FILENAME_DEFAULT,'manufacturers_id='.$manu['manufacturers_id']).'">'.$manu['manufacturers_name'].'</a></li>';
			//}
			//$data = '<div class="box"><div class="box-heading">'.MODULE_BOXES_MANUFACTURERS_BOX_TITLE.'</div><div class="box-content box-category"><ul>'.$manufacturers_string.'</ul></div></div>';
			//$oscTemplate->addBlock($data, $this->group);
		//}
