...
//kotabiru.com custom START//
  function tep_categories_name($id) {
	$cat_query = tep_db_query("SELECT categories_id, categories_name FROM ".TABLE_CATEGORIES_DESCRIPTION." WHERE categories_id = '".(int)$id."'");
	$cat = tep_db_fetch_array($cat_query);
	$cat_name = ((strlen($cat['categories_name']) > MAX_DISPLAY_MANUFACTURER_NAME_LEN) ? substr($cat['categories_name'], 0, MAX_DISPLAY_MANUFACTURER_NAME_LEN) . '..' : $cat['categories_name']);
	$cat_name_edit = str_replace(' ', '-', $cat_name);
	return $cat_name_edit;					
	}
...
function tep_count_reviews_product($products_id){
	$products_query = tep_db_query("select count(*) as total from " . TABLE_REVIEWS . " where products_id = '".(int)$products_id."'");
...
function tep_limiter_word($value,$char='') {
	$result = substr($value, 0, strpos($value, $char));
	return $result;
}
function tep_get_manufacturers_name($manufacturers_id,$model='') {
	 $manu_query = tep_db_query("select manufacturers_id, manufacturers_name, manufacturers_image from " . TABLE_MANUFACTURERS . " where manufacturers_id='".(int)$manufacturers_id."'");
	 $manu = tep_db_fetch_array($manu_query);
	 if($model == 1) {
	 $manu_name = ((strlen($manu['manufacturers_name']) > MAX_DISPLAY_MANUFACTURER_NAME_LEN) ? substr($manu['manufacturers_name'], 0, MAX_DISPLAY_MANUFACTURER_NAME_LEN) . '..' : $manu['manufacturers_name']);
	 $manu_name_edit = str_replace(' ', '-', $manu_name); }
	 else {
		$manu_name_edit = $manu['manufacturers_name'];
	 }
	 return $manu_name_edit;
  }
    function tep_get_category_full_path($category_id) {
	$cPath = '';
	$categories = array();
	...
	function tep_count_products_in_category_manufacturers($category_id, $manufacturer_id, $include_inactive = false) {
    $products_count = 0;
    if ($include_inactive == true) {
      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p.manufacturers_id = '".(int)$manufacturer_id."' and p2c.categories_id = '" . (int)$category_id . "'");
    } else {
      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p.products_status = '1' and p.manufacturers_id = '".(int)$manufacturer_id."' and p2c.categories_id = '" . (int)$category_id . "'");
    }
    $products = tep_db_fetch_array($products_query);
    $products_count += $products['total'];
...
function tep_manufacturers_name($id) {
	$man_query = tep_db_query("SELECT manufacturers_id, manufacturers_name FROM ".TABLE_MANUFACTURERS." WHERE manufacturers_id = '".(int)$id."'");
	$man = tep_db_fetch_array($man_query);
	$man_name = ((strlen($man['nabufacturers_name']) > MAX_DISPLAY_MANUFACTURER_NAME_LEN) ? substr($man['manufacturers_name'], 0, MAX_DISPLAY_MANUFACTURER_NAME_LEN) . '..' : $man['manufacturers_name']);
	$man_name_edit = str_replace(' ', '-', $man_name);
	return $man_name_edit;					
	}  
