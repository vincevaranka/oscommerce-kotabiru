...
// Output a form password field
  function tep_draw_password_field($name, $value = '', $required = false) {
    $field = tep_draw_input_field($name, $value, 'maxlength="40" class="form-control" placeholder="password"', $required, 'password', false);
...
// Output a jQuery UI Button
  ...
  function tep_draw_button_bs($title = null, $icon = null, $link = null, $priority = null, $params = null) {
    static $button_counter = 1;
    ...
    if ( ($params['type'] == 'button') && isset($link) ) {
      $button .= '<a class="btn btn-'.$priority.'"  href="' . $link . '"';

      if ( isset($params['newwindow']) ) {
        $button .= ' target="_blank"';
      }
    } else {
      $button .= '<button class="btn btn-primary" id="tdb' . $button_counter . '" type="' . tep_output_string($params['type']) . '"';
    }
	...
	if ( isset($icon) ) {
       $button .= '><i class="fa fa-'.$icon.'"></i>'.$title;
    } else {
		$button .= '>' . $title;
	}
    if ( ($params['type'] == 'button') && isset($link) ) {
