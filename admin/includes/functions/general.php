...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_info_image($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES_PRODUCT . $image)) ) {
      $image = tep_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES_PRODUCT . $image, $alt, $width, $height);
    } else {
...
// eof Dynamic Template System
 ...
 function tep_info_image_category($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES_CATEGORY . $image)) ) {
      $image = tep_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES_CATEGORY . $image, $alt, $width, $height);
    } else {
      $image = TEXT_IMAGE_NONEXISTENT;
    }
  ...
  function tep_info_image_product($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES_PRODUCT . $image)) ) {
      $image = tep_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES_PRODUCT . $image, $alt, $width, $height);
    } else {
      $image = TEXT_IMAGE_NONEXISTENT;
 ...
 // Sets the status of a product
  function tep_set_information_status($information_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_INFORMATION. " set status = '1' where info_id = '" . (int)$information_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_INFORMATION . " set status = '0' where info_id = '" . (int)$information_id . "'");
    } else {
      return -1;
    }
  }
  // SETS THE STATUS OF FOLLOW US MEDIA SOCIAL
  function tep_set_fs_status($fs_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_FOLLOW_US. " set fs_status = '1' where fs_id = '" . (int)$fs_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_FOLLOW_US . " set fs_status = '0' where fs_id = '" . (int)$fs_id . "'");
    } else {
      return -1;
    }
  }
  function tep_get_information_name($info_id, $language_id = 0) {
    global $languages_id;
    if ($language_id == 0) $language_id = $languages_id;
    $information_query = tep_db_query("select info_name from " . TABLE_INFORMATION_DESCRIPTION . " where info_id = '" . (int)$info_id . "' and language_id = '" . (int)$language_id . "'");
    $information = tep_db_fetch_array($information_query);
    return $information['info_name'];
  }
  function tep_get_information_title($info_id, $language_id = 0) {
    global $languages_id;
    if ($language_id == 0) $language_id = $languages_id;
    $information_query = tep_db_query("select info_title from " . TABLE_INFORMATION_DESCRIPTION . " where info_id = '" . (int)$info_id . "' and language_id = '" . (int)$language_id . "'");
    $information = tep_db_fetch_array($information_query);
    return $information['info_title'];
  }
  function tep_get_information_content($info_id, $language_id = 0) {
    global $languages_id;
    if ($language_id == 0) $language_id = $languages_id;
    $information_query = tep_db_query("select info_content from " . TABLE_INFORMATION_DESCRIPTION . " where info_id = '" . (int)$info_id . "' and language_id = '" . (int)$language_id . "'");
    $information = tep_db_fetch_array($information_query);
    return $information['info_content'];
  }

 // Sets the status of a product
  function tep_set_product_best_status($products_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_PRODUCTS . " set products_best = '1', products_last_modified = now() where products_id = '" . (int)$products_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_PRODUCTS . " set products_best = '0', products_last_modified = now() where products_id = '" . (int)$products_id . "'");
    } else {
      return -1;
    }
  }
  function tep_get_stars($number) {
	$star = '<div class="star">';
	for($i=1; $i<=5; $i++)
	{
		$star .= '<i class="'.(($i<=$number) ? 'staron':'').' fa fa-star"></i>';
	}
	$star .= "</div>";
	return $star;
}
function tep_cfg_input_special($id,$value,$key,$type="text")
{
	$field = '<input type="' . tep_output_string($type) . '" name="configuration[' . $key . ']"  id="'.$id.'" value="' . tep_output_string($value) . '" />';
	$field .= '<script>
				$(function() { $("#'.$id.'").spectrum({flat:false, shopInput: true, preferredFormat: "hex",});
				}); </script>';
	return $field;
}
function tep_get_color($color) {
	$color = '<span style="display:inline-block; width:20px; height:20px; border:1px solid #666; background:'.$color.'"></span>';
	return $color;
}

function tep_info_manufacture_image($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES_MANUFACTURE . $image)) ) {
      $image = tep_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES_MANUFACTURE . $image, $alt, $width, $height);
    } else {
      $image = TEXT_IMAGE_NONEXISTENT;
    }
