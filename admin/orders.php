      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="smallText" align="right"><?php echo tep_draw_button_bs(IMAGE_ORDERS_INVOICE, 'file-text-o', tep_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $HTTP_GET_VARS['oID']), null, array('newwindow' => true)) . tep_draw_button_bs(IMAGE_ORDERS_PACKINGSLIP, 'truck', tep_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $HTTP_GET_VARS['oID']), null, array('newwindow' => true)) . tep_draw_button_bs(IMAGE_BACK, 'chevron-left', tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action')))); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="smallText" valign="top"><?php echo tep_draw_button_bs(IMAGE_UPDATE, 'save', null, 'primary'); ?></td>
          </tr>
        </table></td>
      </form></tr>
<?php
 ...
 <div class="col-md-9 main-content">
	<div class="row margin-bottom">
		...
		<div class="col-xs-6 text-right">
			<?php echo HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_ORDERS)), $orders_statuses), '', 'class="form-control" onchange="this.form.submit();"'); ?>
              <?php echo tep_hide_session_id(); ?></form>
		</div>
	</div>
	<table class="table">
...
?>
	...
	<div class="row">
        <div class="col-xs-6">
			<?php echo $orders_split->display_count($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_ORDERS); ?></div>
        <div class="col-xs-6"><?php echo $orders_split->display_links($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page'], tep_get_all_get_params(array('page', 'oID', 'action'))); ?></div>
    </div>
</div>
<div class="col-md-3">
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button_bs(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button_bs(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id)));
      break;
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        $heading[] = array('text' => '<strong>[' . $oInfo->orders_id . ']&nbsp;&nbsp;' . tep_datetime_short($oInfo->date_purchased) . '</strong>');

        $contents[] = array('align' => 'center', 'text' => tep_draw_button_bs(IMAGE_EDIT, 'edit', tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit')) . tep_draw_button_bs(IMAGE_DELETE, 'trash', tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=delete')));
        $contents[] = array('align' => 'center', 'text' => tep_draw_button_bs(IMAGE_ORDERS_INVOICE, 'file-text-o', tep_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $oInfo->orders_id), null, array('newwindow' => true)) . tep_draw_button_bs(IMAGE_ORDERS_PACKINGSLIP, 'truck', tep_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $oInfo->orders_id), null, array('newwindow' => true)));
