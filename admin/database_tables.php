...
<?php
  if ( isset($action) ) {
    echo '<div style="float: right;">' . tep_draw_button_bs(IMAGE_BACK, 'triangle-1-w', tep_href_link('database_tables.php')) . '</div>';
  }
?>
...
<div class="main" style="text-align: right;">
  <?php echo '<span class="runUtf8" style="display: none;">' . sprintf(ACTION_UTF8_DRY_RUN, tep_draw_checkbox_field('dryrun')) . '</span>' . tep_draw_pull_down_menu('action', $actions, '', 'id="sqlActionsMenu"') . '<span class="runUtf8" style="display: none;">&nbsp;' . tep_draw_pull_down_menu('from_charset', $mysql_charsets) . '</span>&nbsp;' . tep_draw_button_bs(BUTTON_ACTION_GO); ?>
</div>
