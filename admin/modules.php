...
<?php
  if (isset($HTTP_GET_VARS['list'])) {
    echo '' . tep_draw_button_bs(IMAGE_BACK, 'chevron-left', tep_href_link(FILENAME_MODULES, 'set=' . $set)) . '<br /><br />';
  } else {
    echo '' . tep_draw_button_bs(IMAGE_MODULE_INSTALL . ' (' . $new_modules_counter . ')', 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&list=new')) . '<br /><br />';
  }
?>
</div>
</div>
    ...
    <table class="table">
         <thead> 
              ...
              <tr class="dataTableHeadingRow">
				...
				<?php echo (($HTTP_GET_VARS['set']=='boxes') ? '<td class="dataTableHeadingContent">POSITION</td>' : ''); ?>
...
?>
                <td class="dataTableContent"><?php echo $module->title; ?></td>
					<?php echo (($HTTP_GET_VARS['set']=='boxes') ? '<td>'.$module->group.'</td>': ''); ?>
...
?>
		...
		<div class="row">
		<div class="col-md-12">
                <?php echo TEXT_MODULE_DIRECTORY . ' ' . $module_directory; ?></td>
             </div>
		</div>
</div>
<div class="col-md-3">
<?php
  ...
  switch ($action) {
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button_bs(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button_bs(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $HTTP_GET_VARS['module'])));
      break;
    default:
      $heading[] = array('text' => '<strong>' . $mInfo->title . '</strong>');
      ...
      if (in_array($mInfo->code . $file_extension, $modules_installed) && ($mInfo->status > 0)) {
        ...
        $contents[] = array('align' => 'center', 'text' => tep_draw_button_bs(IMAGE_EDIT, 'edit', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=edit')) . tep_draw_button_bs(IMAGE_MODULE_REMOVE, 'minus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove')));
        ...
        if (isset($mInfo)) {
          $contents[] = array('align' => 'center', 'text' => tep_draw_button_bs(IMAGE_MODULE_INSTALL, 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=install')));
