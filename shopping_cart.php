...
<?php
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      $products_name = '<table>' .
                       '  <tr>' .
                       '    <td class="text-left" style="width:200px;"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '">' . tep_image(DIR_WS_IMAGES_PRODUCT . $products[$i]['image'], $products[$i]['name'], 150) . '</a></td>' .
                       '    <td><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '"><strong>' . $products[$i]['name'] . '</strong></a>';
      ...
      $products_name .= '<br /><br />' . tep_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="4" class="form-control"') .'<br />'. tep_draw_hidden_field('products_id[]', $products[$i]['id']) . tep_draw_button_booth(IMAGE_BUTTON_UPDATE, 'refresh') . '&nbsp;&nbsp;&nbsp;' . TEXT_OR . '&nbsp;&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_SHOPPING_CART, 'products_id=' . $products[$i]['id'] . '&action=remove_product') . '" class="btn btn-default">' . TEXT_REMOVE . '</a>';
...
?>
 ...
 <?php echo tep_draw_button_booth(IMAGE_BUTTON_CHECKOUT, 'glyphicon glyphicon-ok', tep_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'), 'primary'); ?><br /><br />
<?php
